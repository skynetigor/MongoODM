...
{
    internal class AddedDataChangesSaver<TEntity> : AbstractChangesSaver<TEntity>
    {
        private static MethodInfo SetRelationsMethod { get; }
        private IDbsetContainer DbsetContainer { get; }

        static AddedDataChangesSaver()
        {
            SetRelationsMethod = typeof(AddedDataChangesSaver<TEntity>).GetMethod(nameof(SetRelations), BindingFlags.NonPublic | BindingFlags.Instance);
        }

        public AddedDataChangesSaver(IMongoDatabase database, ITypeInitializer typeInitializer, IDbsetContainer dbsetContainer) : base(database,
            typeInitializer)
        {
        ...
        {
            Database.GetCollection<TEntity>(CurrentTypeModel.CollectionName).InsertMany(dataForSaving);
        }
